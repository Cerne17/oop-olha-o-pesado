...
from computer.vision.detection_utils import (
    process_image,
    identify_gesture,
...
from computer.vision.image_utils import (
    write_gesture_name,
    draw_squeleton,
    draw_person_box,
)
    ...
    def Run(self):
        ...
        with HandLandmarker.create_from_options(self.options) as self.landmarker:
            while not self.finish:
                ...
                img = cv2.imdecode(
                    np.frombuffer(self.latest_frame.jpeg, np.uint8), cv2.IMREAD_COLOR
                )
                if img is None:
                    continue
    ...
    def ProcessImage(self, img, img_rgb):
        ...
        for person in persons_data:
            ...
            if self.alvo_rastreado_id is None:
                ...
                if (
                    self.tempo_perda_alvo is None
                    and gestos_dessa_pessoa.count("ROCK") == 2
                ):
            ...
            # ESTADO 2: Focado no mestre
                ...
                ref = ControlSignal.following(
                    angle=angulo_calculado, speed=velocidade_calculada
                )
                ...
                if (
                    gestos_dessa_pessoa.count("ROCK") == 1
                    and gestos_dessa_pessoa.count("STOP") == 1
                ):
                    self.alvo_rastreado_id = None
                    self.tamanho_alvo_ref = (
                        None  # Limpa a referência de tamanho do alvo
                    )
