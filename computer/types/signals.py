...
@dataclass(frozen=True)
class ControlSignal:
    angle_deg: float = 0.000  # [-180.0, 180.0] — 0°=forward, 90°=pivot right
    speed_ref: float = (
        0.000  # [-1.0, 1.0]     — +1=max forward, 0=stop, -1=max backward
    )
    buzzer: bool = False  # False: off (silence); True: on (sound)
    leds: int = 0b001  # one-hot codification: 0b000 (all off), 0b001 (yellow on), 0b010 (green on), 0b100 (red on)
